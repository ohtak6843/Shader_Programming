...
void Renderer::Initialize(int windowSizeX, int windowSizeY)
{
	...
	m_ParticleTexture = CreatePngTexture("./Textures/purple.png", GL_NEAREST);
...
void Renderer::CreateFBOs()
{
	for (int idx = 0; idx < 2; idx++)
	{
		...
		glGenTextures(1, &m_SubRTs[idx]);
		glBindTexture(GL_TEXTURE_2D, m_SubRTs[idx]);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
		...
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, m_SubRTs[idx], 0);
...
{
	// 1. render to HDRFBO0 (rt: HDRRT0_0, HDRRT0_1)
	glBindFramebuffer(GL_FRAMEBUFFER, m_HDRFBO0);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	glViewport(0, 0, 512, 512);
	DrawParticle();
...
void Renderer::CreateParticles(int count)
{
	...
	int floatCountsPerVertex = 3 + 1 + 4 + 1 + 3 + 1 + 1 + 1 + 2;
